...
{$ELSE}
	LCLIntf, LCLType, LMessages, GraphType,
  	Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, 
	ExtCtrls, Menus, ActnList, ImageToTilesTypes;
{$ENDIF}
...
procedure TImageViewMainForm.DoClear;
	var
	i: Integer;

	begin
	FillChar(FHeader, SizeOf(TV4TImgHeader), 0);
	FillChar(FBackground, SizeOf(TRGBPixel), 0);

	for i:= FTiles.Count - 1 downto 0 do
		FTiles[i].Free;
	FTiles.Clear;

	for i:= 0 to High(FScreen) do
		SetLength(FScreen[i], 0);
	SetLength(FScreen, 0);
...
{$ELSE}
					...
					Inc(PByte(PixelRowPtr), RawImage.Description.BytesPerLine *
							(ty * VAL_SIZ_TILEPIX_LENGTH + i));
					PixelPtr:= PixelRowPtr;
					Inc(PByte(PixelPtr), BytePerPixel *
							tx * VAL_SIZ_TILEPIX_LENGTH);
					l:= PRGBPixel(PixelPtr);
{$ENDIF}
					for j:= 0 to VAL_SIZ_TILEPIX_LENGTH - 1 do
						begin
